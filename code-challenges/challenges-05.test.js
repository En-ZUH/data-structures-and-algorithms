...
const toLastNames = people => {
  // Solution code here...
  let newArr = [];
  people.map(fullName => newArr.push(`${fullName.firstName} ${fullName.lastName}`));
  return newArr;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return arr.reduce((acc, value, index) => index + 1, 0);
};
...
const returnNames = (arr) => {
  ...
  //  return arr.reduce((acc, value, index) => acc.push(value.name), []);
  return arr.reduce((acc, value, index) => {
    // acc.push(arr[index].name);
    acc.push(value.name);
    return acc;
  }, []);
};
...
const extractStat = (statName, arr) => {
  // Solution code here...
  return arr.reduce((acc, value, index) => value.stat.name === statName ? value : acc, null);
};
...
const extractChildren = (arr) => {
  ...
  // return arr.filter(character => character.name.includes('a')).reduce((acc, value, index) => {
  //   if (value.children)
